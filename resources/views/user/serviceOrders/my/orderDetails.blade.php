    ...
    <section class="content">
        ...
        <div class="printableArea">    
            ...
            <table class="table_invoice table_invoice-condensed table_invoice-bordered table_invoice-striped" style="margin-bottom: 5px;" cellpadding="0" cellspacing="0" border="0">
                ...
                <th style="width:33% !important;">Item List</th>
                <th style="width:12.5% !important; text-align:center;">Quantity</th>
                <th style="width:12.5% !important; text-align:center;">Color</th>
                <th style="width:12.5% !important; text-align:center;">Size</th>
                <th style="width:12.5% !important; text-align:center;">Sale Price</th>
                <th style="width:13% !important; text-align:center;">Total Price</th>
                </tr>
                </thead>
                <tbody>
                    <?php $total_price = 0; $i=1; ?>
                    @foreach ($order->orderItems as $item)
                        <tr>
                            <td class="text-md-center">{{$i++}}</td>
                            <td class="text-md-left"><strong>{{ $item->product->name }}</strong></td>
                            
                            <td class="text-md-left">{{ $item->quantity }} {{ $item->product->unit->name ?? ''}}</td> 
                            @if($item->product->variation != Null)  
                            <td class="text-md-left">{{ $item->color->name ?? ''}}</td>   
                            <td class="text-md-left">{{ $item->size->name ?? ''}}</td>
                            @else
                            <td></td>
                            <td></td>
                            @endif
                            <td class="text-md-right">BDT: {{ $item->sale_price }}</td> 
                            <td class="text-md-right">BDT: {{ $item->total_sale_price }}</td>   
                        </tr> 
                ...
                <tr>
                    ...
                    @endphp
                <td colspan="7"><span style="font-weight: bold;">In Word: {{$f->format($total_price )}}</td>
                </tr>
                <tr>
                  <td colspan="5"><span style="font-weight: bold;">Note: Without Delivery Charge.</td>
                </tr>
                </tfoot>    
                </table> 
            </div>    
                                        ...
                                        @foreach ($order->orderItems as $item)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <input type="hidden" name="cat_id" value="{{ $item->ws_cat_id }}">
                                                ...
                                                {{-- <td>
                                                    <form class="form-inline" method="post"
                                                        ...
                                                        <div class="input-group mb-3 input-group-sm">
                                                            <select class="form-control" name="order_status" id="order_status">
                                                                <option {{ $item->order_status =="pending"?'selected':'' }} value="pending">Pending</option>
                                                                <option {{ $item->order_status =="confirmed"?'selected':'' }} value="confirmed">Confirmed</option>
                                                                <option {{ $item->order_status =="cancelled"?'selected':'' }} value="cancelled">Cancelled</option>
