...
@section('content')
    ...
    <section class="content">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <h4 class="m-auto text-center">Product Information</h4>
                        ...
                        <tbody>
                            ...
                            <form action="{{ route('subscriber.updateCartProduct',['cart'=>$item->id]) }}" method="POST">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="hidden" name="variation" value="{{ $item->product->variation }}">

                                        @if($item->product->variation=='1')
                                        @php 
                                        $variation=\App\Models\ServiceProductVariation::where('proid',$item->product->id)->get();
                                            
                                        @endphp

                                            <select name="color" id="color" >
                                               @foreach ($variation as $data)
                                               <option value="{{$data->colid}}" @if($data->colid==$item->color) selected @endif  >{{$data->color->name ?? ''}}</option>   
                                               @endforeach
                                    ...
                                    <td>
                                        @if($item->product->variation=='1')
                                            <select name="size" id="size" >
                                               @foreach ($variation as $data)
                                               <option value="{{$data->sizid}}" @if($data->sizid==$item->size) selected @endif>{{$data->size->name ?? ''}}</option>   
                                               @endforeach
                ...
                <form
                    action="{{ route('subscriber.serviceProductsOrderSubmit', ['profile' => $profile->id, 'subscription' => $subscription->subscription_code]) }}"
                    method="post">
                    @csrf
                    ...
                    <div class="form-group">
                          <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="paymod" id="flexRadioDefault1" value="cashon">
