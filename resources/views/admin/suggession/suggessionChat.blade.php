                                    ...
                                    <h3 class="card-title">
                                        ...
                                        @else
                                         @if(Auth::user()->isAdmin())
                                          <a href="{{ route('user.OpenSuggesionsPost',['parent'=>$suggession->id]) }}" class="badge badge-danger">Open Now</a>
                                         @endif
                                        @endif
                                ...
                                <div class="card-body" style="display: block;">
                                    <div class="direct-chat-messages">
                                        @foreach ($suggession->suggessionDiscuss($suggession->id) as $sg)
                                            @if ($sg->admin_id == null)
                                                <div class="direct-chat-msg left userChat">
                                            ...
                                            @else
                                                <div class="direct-chat-msg right">
                                                    <div class="direct-chat-infos clearfix">
                                                        <span
                                                            class="direct-chat-name float-left">Soft Commerce</span>
                                                        <span
                                                            class="direct-chat-timestamp float-right">{{ \Carbon\Carbon::parse($sg->created_at)->format('d M, Y') }}</span>
                                                    </div>
                                                    <!-- /.direct-chat-infos -->
