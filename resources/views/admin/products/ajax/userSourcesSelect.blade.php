<div class="card card-widget">
    ...
    @foreach($product->user->sources as $source)    

    <div class="form-check">
      <label class="form-check-label">
        <input type="radio" class="form-check-input source-selected" name="source" value="{{ $source->id }}" {{ $product->source_id == $source->id ? 'checked' : '' }}>{{ $source->name }} ({{ $source->type }}) 
        {{-- ({{ $source->market->name }}) --}}
      </label>
    </div>
    ...
    {{-- <div class="source-change-url-get" data-url="{{ route('admin.leadSourceChange', ['product' =>$product]) }}" style="display: none;"></div> --}}
    <div class="source-change-url-get"  style="display: none;"></div>
    <br>

     <a class="toggle-btn-for-lead-source-add btn btn-link" href="">	<i class="fa fa-edit"></i>	 {{__('Add lead source information')}}</a>

     <div class="add-new-lead-source-info float-left w-100" style="display: none;">
     	<br>
     	{{-- <form method="POST" class="new-source-create-form" action="{{ route('admin.leadSourceCreatePost', ['lead'=>$lead]) }}"> --}}
            <form method="POST" class="new-source-create-form" action="">
    @csrf

    <div class="form-group mb-1 row">
        <label for="title" class="col-md-4 col-form-label text-md-right">{{ __('Name or Title') }}*</label>

        <div class="col-md-6">
            <input id="title" type="text" class="form-control input-for-new-user form-control-sm @error('title') is-invalid @enderror" name="title" value="{{ old('title') }}" placeholder="{{ __('Name or Title') }}" required autocomplete="off" autofocus>
        </div>
    </div>
