        ...
        <div class="container">
            <div class="row">
                <div class="col-md-5 m-auto">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('admin.updateunit',$data->id) }}" method="POST" id='my-form'>
                                @csrf
                                ...
                                <div class="form-group">
                                   ...
                                   <select name="cat_id" id="cat_id" class="form-control">
                                       ...
                                       @foreach ($categories as $cat)
                                          <option value="{{ $cat->id}}" @if($data->cat_id==$cat->id) Selected @endif>{{$cat->name}}</option> 
                                       @endforeach
                                   </select>
                                    @error('type')
                                        <span class="text-danger">{{ $message }}</span>
