        ...
        <div class="container">
            <div class="row">
                <div class="col-md-5 m-auto">
                    <div class="card">
                        <div class="card-body">
                            <form action="{{ route('admin.socialGroupsUpdate',['social'=>$socialGroup->id]) }}" method="POST">
                                @csrf
                                <div class="form-group">
                                    <label for="type">Group Type</label>
                                    <select name="type" id="" class="form-control">
                                        <option value="">Select Group Type</option>
                                        <option {{ $socialGroup->type == 'facebook' ? "selected" :null }} value="facebook">Facebook</option>
                                        <option {{ $socialGroup->type == 'youtube' ? "selected" :null }} value="youtube">Youtube</option>
                                        <option {{ $socialGroup->type == 'others' ? "selected" :null }} value="others">Others</option>
                                    </select>
                                    @error('type')
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title"  class="form-control" value="{{ $socialGroup->title }}"> 
                                    @error('name')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" name="link"  class="form-control" value="{{ $socialGroup->link }}">
                                    @error('link')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    <label for="active">
                                        <input type="checkbox" checked name="active" {{ $socialGroup->active == true ? 'checked' :null }} id=""> Active
                                    </label>
                                </div>
                                <div class="form-group">
