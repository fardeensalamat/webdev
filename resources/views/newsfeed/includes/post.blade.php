...
<div class="card gedf-card">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text" style="white-space:pre-wrap;">{{ucfirst($post->description)}}</p>
                    <div class="row">
                ...
                <div class="card-footer bg-white w3-border-top w3-border-light-gray">
                        ...
                        <span class="likeArea">
                            @includeIf('user.ajax.like.postLikeArea')
                        </span>
 ...
 <div class="commentArea">
                ...
                <span class="previousComment">
                  <div class="loadingPrevCom pull-right" style="display: none; padding-right: 80px;"><span class='fa fa-circle-o-notch fa-spin w3-text-deep-orange'></span> Loading...</div>
                  <?php $takeNumber = 4; ?>
                  @if($post->comments->count() > $post->selectedComments($takeNumber)->count())
                  <a class="pull-right previousDiscusses" 
                  {{-- href="{{route('seePreviousComForPost', ['post'=>$post->id])}}" --}}
                  >
                  <i class="fa fa-comments-o" aria-hidden="true"></i>
                  View Previous {{$post->comments->count() - $takeNumber}} {{Str::plural('Comment', ($post->comments->count() - $takeNumber))}}</a><br/>
                  @endif
                </span>
                  @foreach($post->selectedComments($takeNumber) as $comment)
                    @include('user.ajax.comment.commentSingle')
                  @endforeach
              ...
              <div class="loadingNewComment" style="display: none; padding-left: 40px;"><span class='loadNewComment fa fa-sync fa-spin w3-text-deep-orange'></span> Loading...</div>

              <div class="card-footer">
    <form action="{{route('user.commentCreate',['type'=>'post','id' =>$post->id])}}" method="post">
    {{csrf_field()}}
      <img class="img-responsive img-circle img-sm" src="{{ route( 'imagecache', ['template'=>'ppmd', 'filename' => Auth::user()->fi() ]) }}" alt="alt text">
      <!-- .img-push is used to add margin to elements next to floating images -->
      <div class="img-push">
      <!-- <input type="text" name="comment_body" class="form-control input-sm" placeholder="Press enter to post comment"> -->

      <textarea class="form-control comment_box comment-box" name="comment_body" rows="1" placeholder="Type Your Comment and Press Enter" data-url="{{route('user.commentCreate',['type'=>'post','id' =>$post->id])}}"></textarea>

      </div>
      <button class="btn btn-xs comment_submit" type="submit" style="display: none;">submit your comment</button>
    </form>
  </div><!-- /.box-footer -->
