                ...
                <aside class="sidebar">
                    ...
                    <ul class="nav nav-list flex-column mb-5">
                        @forelse ($categories as $cat)
                            <li class="nav-item"><a class="nav-link" href="#">{{ $cat->name }}
                                    ({{ $cat->count('post') }})</a></li>
                        @empty
                        ...
                        <article class="post post-large">
                            <div class="post-image">
                                <a
                                    href="
                                    @if ($type == 'news')
                                    {{ route('welcome.newsDetails', ['title' => mySlug($blog->title), 'news' => $blog->id]) }}
                                    @elseif ($type == 'blog')
                                    {{ route('welcome.blogDetails', ['title' => mySlug($blog->title), 'blog' => $blog->id]) }}
                                    @elseif($type == 'event')
                                    {{ route('welcome.eventDetails', ['title' => mySlug($blog->title), 'event' => $blog->id]) }}
                                    @endif
                                    ">
