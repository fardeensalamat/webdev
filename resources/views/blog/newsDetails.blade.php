            ...
            <div class="row">
                ...
                <div class="col-md-4 order-1 order-md-2 align-self-center">
                    <ul class="breadcrumb d-block text-md-right breadcrumb-light">
                        <li><a href="/">Home</a></li>
                        <li class=""> <a href="{{ route('welcome.blog', ['type' => 'news']) }}">News</a> </li>
                        <li class="">{{ mb_substr($news->title, 0, 30) }}</li>
                    </ul>
                </div>
            </div>
        </div>
                ...
                <div class="blog-posts single-post">
                    ...
                    <article class="post post-large blog-single-post border-0 m-0 p-0">
                        <div class="post-image ml-0">
                            <a href="">
                                <img src="{{ route('imagecache', ['template' => 'fifb', 'filename' => $news->fi()]) }}"
                        ...
                        <div class="post-date ml-0">
                            <span
                                class="day">{{ \Carbon\Carbon::parse($news->created_at)->format('d') }}</span>
                            <span
                                class="month">{{ \Carbon\Carbon::parse($news->created_at)->format('M') }}</span>
                        </div>
                        ...
                        <div class="post-content ml-0">
                            ...
                            <div class="post-meta">
                                <span><i class="far fa-user"></i> By <a
                                        href="#">{{ $news->user ? $news->user->name : '' }}</a> </span>
                                <span><i class="far fa-folder"></i>
                                    @if ($news->blogCategories)
                                        @foreach ($news->blogCategories as $bcat)
                                            <a
                                ...
                                <span><i class="fas fa-tags"></i>
                                    ...
                                    @if ($tags)
                                        @foreach ($tags as $tag)
                                        <a
                                        href="{{ route('welcome.tagWisePost', ['type' => 'news','tag'=>1, 'title' => $tag]) }}">{{ $tag }}</a>
                                        @endforeach
                                    @endif
                            ...
                            <div class="post-block mt-4 pt-2 post-author">
                                ...
                                <div class="img-thumbnail img-thumbnail-no-borders d-block pb-3">
                                    <a href="#">
                                        <img src="{{ route('imagecache', ['template' => 'ppmd', 'filename' => $news->user->fi()]) }}"
                                            alt="">
                                    </a>
                                </div>
                                <p><strong class="name"><a href="#"
                                            class="text-4 pb-2 pt-2 d-block">{{ $news->user ? $news->user->name : null }}</a></strong>
                                </p>
